...
export function PrismaAdapter(
  ...
  return {
    async createUser({ name, email, image, emailVerified }) {
      const {
        "@will-have-a-name:additionalUserData": additionalDataOnCookies,
      } = parseCookies({
        req,
      });
      if (!additionalDataOnCookies && !name)
        throw new Error("No user name found");

      const userAdditionalData = JSON.parse(additionalDataOnCookies || "") as {
        name: string;
        phoneNumber: number;
      };
